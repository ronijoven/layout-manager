...
import Droppable from '../Virtualize/droppable';
import Draggable from '../Virtualize/draggable';
import DragDropContext from '../Virtualize/drag_drop_context';
...
class Example3 extends Component {
	...
	sendData = (parentData) => {
		this.props.rest.parentCallback(parentData);
	};
		   ...
		   <Row>
			...
			<  Col>
				<div className={'input-section'}>
					Items per column
					<input
						defaultValue={this.state.numItems}
						style={{marginLeft: 20, marginTop: 8, marginBottom: 8, padding: 2}}
						placeholder={100}
			    ...
			    <div className={'test-container'} style={{display: 'flex', flexDirection: 'row', position: 'relative'}}>
					{elemsToRender.map((elem, index) =>
						!this.state.split || index < elemsToRender.length / 2 ? (
							<div className={'sizer'} style={{flexGrow: 1, minWidth: 100}} key={index + elem.droppableId}>
								<Droppable
				...
				<div className={'test-container'} style={{display: 'flex', flexDirection: 'row', position: 'relative'}}>
					{elemsToRender.map((elem, index) =>
						index >= elemsToRender.length / 2 ? (
							<div className={'sizer'} style={{flexGrow: 1, minWidth: 150}} key={index + elem.droppableId}>
								<Droppable
									activeHeaderClass={'header-active'}
									listHeader={getListHeader(index)}
									listHeaderHeight={60}
